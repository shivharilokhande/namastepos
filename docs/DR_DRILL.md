...
| **R2 `namastepos-downloads/NamastePOS.apk`** | ❌ Only other copies are the in-tree `NamastePOS.apk` and your local build dir | Losing it 404s every customer download link. Keep the release APK in the repo (it already is) or a GitHub Release — the rebuild path is `flutter build apk --release` then `wrangler r2 object put`. |
