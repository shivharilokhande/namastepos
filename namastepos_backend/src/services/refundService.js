...
async function refundOrder({
  ...
  // NP-303 (2026-09-04) — SESSION-level lock before the cap is computed.
  //
  // The order-row `FOR UPDATE` below serialises two refunds against the SAME
  // order, but for a multi-KOT table session the cap is computed over ALL
  // orders in the session (see the bill_total / prior-refunds queries further
  // down, added by the H2 fix). Two refunds aimed at DIFFERENT orders of one
  // session therefore locked different rows, each read "previous refunds = 0",
  // and both passed — over-refunding the session, real money out.
  //
  // Fix: when the order belongs to a session, take ONE lock on the session row
  // first, so concurrent refunds anywhere in that session serialise on it and
  // the second one sees the first one's row. This pre-read is unlocked and only
  // reads `table_session_id`, which never changes for an order, so it cannot be
  // stale in a way that matters. Lock order is always session → order, so two
  // refunds in the same session cannot deadlock. The single-order path is
  // untouched: no session id, no extra lock, one cheap indexed read.
    const sessionProbe = await client.query(
      'SELECT table_session_id FROM orders WHERE id = $1 AND business_id = $2 LIMIT 1',
      [orderId, businessId],
    );
    const probedSessionId = sessionProbe.rows[0]?.table_session_id || null;
    if (probedSessionId) {
      await client.query(
        'SELECT id FROM table_sessions WHERE id = $1 FOR UPDATE',
        [probedSessionId],
      );
    }
    ...
    LEFT JOIN menu_items mi ON mi.id = oi.menu_item_id
      ...
      for (const it of items) {
        ...
        if (!row) continue;
        const remainingQty = Math.max(0, parseFloat(row.qty) - (refundedQty.get(it.id) || 0));
    ...
    if (_gatewayRzPaymentId) {
      try {
        const rz = await module.exports._rzCall(
          'POST',
          `/v1/payments/${_gatewayRzPaymentId}/refund`,
