...
describe('Receipt rendering', () => {
  const sampleOrder = {
    orderNo: 42, source: 'dineIn', tableNo: '3',
    customerPhone: '9876543210', subtotal: 600,
    serviceChargeInr: 30, cgst: 15, sgst: 15, igst: 0, tax: 30,
    discount: 0, roundOffInr: 0, total: 660,
    paymentMethod: 'cash', createdAt: new Date('2026-05-20T13:00:00').toISOString(),
    tokenNo: 47,
  };
  const sampleItems = [
  ...
  test('80mm receipt has header + items + total', () => {
    const out = renderReceiptText({
      ...
      order: sampleOrder, items: sampleItems,
    });
  ...
  test('duplicate flag prints DUPLICATE banner', () => {
    const out = renderReceiptText({
      ...
      order: sampleOrder, items: sampleItems, isDuplicate: true,
    });
