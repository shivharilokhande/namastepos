...
async function walletBal(customerId) {
  return parseInt((await query(
    `SELECT balance_paise FROM customer_wallets WHERE customer_id = $1`, [customerId]
  )).rows[0].balance_paise, 10);
}
async function legs(orderId) {
  const r = await query(
    `SELECT method, amount_paise FROM payments WHERE order_id = $1 ORDER BY method`, [orderId]
  );
...
describe('Wallet auto-apply after membership', () => {
  it('wallet partially covers the residual; rest to cash', async () => {
    ...
    const order = await orderService.create(biz.id, {
      source: 'takeaway', customerPhone: '9700000001', paymentMethod: 'cash',
      items: cart(item.id), autoWallet: true,
    });
  ...
  it('wallet fully covers the residual; balance left over; no cash leg', async () => {
    ...
    const order = await orderService.create(biz.id, {
      source: 'takeaway', customerPhone: '9700000002', paymentMethod: 'cash',
      items: cart(item.id), autoWallet: true,
    });
  ...
  it('respects the cashier cap', async () => {
    ...
    const order = await orderService.create(biz.id, {
      source: 'takeaway', customerPhone: '9700000003', paymentMethod: 'upi',
      items: cart(item.id), autoWallet: true, walletCapInr: 30, // cap ₹30
    });
