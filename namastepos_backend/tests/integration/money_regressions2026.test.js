...
describe('Razorpay: cancelled subscription is not resurrected by a charge', () => {
  it('records the payment but keeps the sub cancelled, with a sequence invoice no', async () => {
    ...
    const sub = (await query('SELECT status FROM subscriptions WHERE business_id = $1', [biz.id])).rows[0];
    expect(sub.status).toBe('cancelled'); // guard held — NOT reactivated

    const inv = (await query('SELECT number, amount_paise FROM invoices WHERE business_id = $1', [biz.id])).rows[0];
...
describe('Guest path: membership bundle only spent when benefits allowed', () => {
  it('does NOT decrement a member bundle when allowMemberBenefits is false', async () => {
    ...
    await orderService.create(biz.id, {
      source: 'other',
      customerPhone: phone,
      allowMemberBenefits: false,
      items: [{ menuItemId: item.id, name: 'Cold Coffee', price: 150, qty: 2 }],
    });
    let rem = (await query('SELECT remaining FROM membership_subscriptions WHERE customer_id = $1', [cust.id])).rows[0].remaining;
    ...
    rem = (await query('SELECT remaining FROM membership_subscriptions WHERE customer_id = $1', [cust.id])).rows[0].remaining;
...
describe('Order cancel restores raw-ingredient stock', () => {
  it('adds ingredient stock back on cancel, not just dish stock', async () => {
    ...
    const afterCreate = (await query('SELECT stock FROM ingredients WHERE id = $1', [ing.id])).rows[0].stock;
    ...
    const afterCancel = (await query('SELECT stock FROM ingredients WHERE id = $1', [ing.id])).rows[0].stock;
    expect(Number(afterCancel)).toBe(1000); // fully restored
    // And a 'returned' ingredient ledger row exists.
    const ret = (await query(`SELECT count(*)::int AS c FROM ingredient_transactions
        WHERE order_id = $1 AND kind = 'reverse'`, [order.id])).rows[0].c;
